...
The average length of the movies in the data set is about 82 minutes and 34 seconds. The standard deviation was of about 44 minutes and about 35 seconds. The oldest movies on this list were made in 1893.
...
The data shows that action movies are often longer with run times of about 100 minutes compared to non-action movies having run times of about 81 minutes. The standard deviation also shows the difference between the two as it is lower, with the Action movies earning more. 
...
Our data states that the short movies have an average length of about 14 minutes, while non-short movies have an average length of about 95 minutes. Interestingly enough, Short and non-short movies have the same lowest run time for lowest length of a movie, both being 1 minute as the minimum length of a movie.
...
```{r}
movies_SdQ<- movies_lin128 %>%
filter(per_low > 50 & votes > 5)%>%
mutate(length_hrs = length / 60) %>%
summarize(mean (length_hrs))
movies_SdQ
```

The movies with a low rating and votes greater than 5 have an average length in hours of about 1.5.
...
When flipping both data sets of mutate and summarize we were able to be done as seen by the new data sets of average_length_hours and average_length_hours_flipped. Using mutate first leads to a more straight forward answer as it gives us the average length in hours right away when we need it, using summarize first then mutate shows the process that lead to the data which has its uses for tracking progression.
